...
func NewFileCollector(cfg *FileCollectorConfig) (*FileCollector, error) {
	...
	if err != nil {
		log.Println("[ERR] mkdir failed:", cfg.LogDir, err)
		return nil, err
	}
	rl, err := OpenRolloger(cfg.LogDir, cfg.ChunkBits, RollChunks)
	if err != nil {
		log.Println("[ERR] rollog.Open failed -", err.Error())
		return nil, err
	}
	fc := &FileCollector{
		rl:    rl,
...
func (fc *FileCollector) persistOne(s *Span) {
	...
	if err = fc.rl.SafeLog(data); err != nil {
		log.Println("[WRN] persist fail:", err)
	}
}
...
func tryCleanHistory(dir, service string) {
	f, err := os.Open(dir)
	if err != nil {
		log.Println("[WRN] open dir fail:", err)
		return
	}
	list, err := f.Readdir(-1)
	if err != nil {
		log.Println("[WRN] read dir fail:", err)
		return
	}
	f.Close()

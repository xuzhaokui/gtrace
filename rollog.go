...
func OpenRolloger(name string, chunkBits, keepn uint) (r *Rolloger, err error) {

	err = syscall.Mkdir(name, 0777)
	if err != nil {
		if err != syscall.EEXIST {
			err = errors.New("rollog.Open failed: " + err.Error())
			return
		}
		err = nil
	}
	...
	if chunkBits > 32 {
		err = errors.New("rollog.Open failed: invalid argument")
		return
	} else if chunkBits == 0 {
...
func (r *Rolloger) getCurrent() error {
	...
	if err != nil {
		err = errors.New("rollog.readDir failed: " + err.Error())
		return err
	}
